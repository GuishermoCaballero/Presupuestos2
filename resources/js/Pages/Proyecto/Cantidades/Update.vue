...
const { proyecto, gasto } = usePage().props;

const selectedOption = ref("cambio"); // Set the default selected option

if(gasto.anadir_iva == 1 ){
  gasto.anadir_iva = true
}else{
  gasto.anadir_iva = false
}

const cambio_form = useForm({
  id: gasto.id,
  factura_id: gasto.id_factura,
  nombre: gasto.nombre,
...
const cambio_submit = () => {
  cambio_form.post(route("proyecto.cantidad.save", { id: gasto .id }));
};
            ...
            <form @submit.prevent="cambio_submit">
              ...
              <div class="block my-4">
                  <label class="flex items-center">
                      <Checkbox name="anadir_iva" v-model="cambio_form.anadir_iva" :checked="cambio_form.anadir_iva" />
              ...
              <div class="flex items-center justify-end mt-4">
                <Link
                  :href="route('proyecto.gasto.delete', { id: gasto.id })"
                  method="delete"
                  as="button"
